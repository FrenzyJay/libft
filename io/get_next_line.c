...
/*   Updated: 2014/03/18 06:26:36 by garm             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*ft_gnl_cut(char *ref, char **s1, char **s2)
{
	int		size1;
	int		size2;

	size1 = ft_strlenc(ref, '\n');
	size2 = ft_strlen(ref) - size1 - 1;
	if (size2 < 0)
		size2 = 0;
...
int				get_next_line(int const fd, char **line)
{
	...
	if ((ret == -1) || (ret == 0 && **memory == '\0'))
		return (ret);
	tmp = ft_gnl_cut(*memory, line, memory);
	ft_strdel(&tmp);
	return (1);
}
